...
-- ------------------------------------------------------
-- DML(Data Manipulator Language, 데이터 조작어)
-- ------------------------------------------------------
...
DELETE FROM dept WHERE deptno IN (50, 60, 70, 80);
...
-- ROLLBACK; -- 트랜잭션의 시작하기 이전상태로 되돌려라!!
COMMIT; -- 영구적으로 위의 테이블의 변경을 저장하라!!!
...
-- 1) INTO 절에 명시된 컬럼의 갯수와 VALUES 절에 명시된 값의 
--    개수가 틀린 경우
INSERT INTO dept (deptno, dname, loc)
...
CREATE TABLE myemp_hire     -- 1st. Table creation.
AS
...
CREATE TABLE myemp_mgr      -- 2nd. Table creation.
AS
...
CREATE TABLE myemp_hire2     -- 1st. Table creation.
AS
SELECT empno, ename, hiredate, sal
FROM emp
WHERE 1 = 2;        -- 기존 테이블의 스키마만 복사(데이터 제외)
...
CREATE TABLE myemp_mgr2      -- 2nd. Table creation.
AS
SELECT empno, ename, mgr
FROM emp
WHERE 1 = 2;        -- 기존 테이블의 스키마만 복사(데이터 제외)
...
CREATE TABLE myemp_hire3     -- 1st. Table creation.
AS
SELECT empno, ename, hiredate, sal
FROM emp
WHERE 1 = 2;        -- 기존 테이블의 스키마만 복사(데이터 제외)
...
CREATE TABLE myemp_mgr3      -- 2nd. Table creation.
AS
SELECT empno, ename, mgr
FROM emp
WHERE 1 = 2;        -- 기존 테이블의 스키마만 복사(데이터 제외)
...
--      사용된 컬럼명이 반드시 동일해야 함.
-- ------------------------------------------------------
TRUNCATE TABLE myemp_hire3;
TRUNCATE TABLE myemp_mgr3;
