...
--    b. 생략가능:
--       기본테이블(Base tables)의 컬럼명 또는 
--      서브쿼리의 SELECT절의 컬럼별칭 사용
--  * Sub-query:
...
-- 권한할당: DCL (Data Control Language)
--          1) GRANT 문(권한할당)   2) REVOKE 문(권한회수)
...
CREATE OR REPLACE VIEW emp_view2     -- OK: View 객체수정
AS
...
CREATE OR REPLACE VIEW emp_view3 (사원번호, 이름, 월급)    -- Alias 사용
AS
...
CREATE OR REPLACE VIEW emp_view5 (deptno, 부서원수)
AS
SELECT 
    deptno, count(*) 
FROM emp
...
-- 단순 뷰(Simple View) 데이터에 대한 DELETE 문
-- 에러발생: ORA-01732: data manipulation operation not legal 
--                     on this view
DELETE FROM emp_view5
WHERE deptno = 40;
