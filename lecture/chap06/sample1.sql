...
SELECT  -- 메인쿼리
    ...
    (   -- 상관부속질의: 메인쿼리의 결과값을 사용해야만 수행가능한 쿼리
        SELECT t.first_name 
        FROM employees t 
        WHERE t.employee_id = m.employee_id -- <- 메인쿼리가 주는 값
    ) AS first_name,
...
FROM
    ...
    -- 비상관부속질의
    (SELECT * FROM employees WHERE last_name != 'Whalen') m
WHERE
    -- 2. Whalen 보다 많은 월급여를 받는 사원조회
    salary >= (
        -- 서브쿼리(= 비상관 부속질의)
        -- 단독 수행에 지장없음
        ...
        SELECT salary
        FROM employees
        WHERE last_name = 'Whalen'  -- 1. Whalen 의 월급여 획득
    );
