...
WHERE
    ...
    --
    --          복수행 서브쿼리가 반환한 모든 값 중, *최소값 보다 작은* 사원 필터링
    salary < ALL (
...
WHERE
    ...
    --          메인쿼리 조건을 수행가능하도록 ALL 키워드 적용
    --
    --          복수행 서브쿼리가 반환한 모든 값 중, *최대값 보다 큰* 사원 필터링
    salary > ALL (
        -- 복수 행 비상관 서브쿼리: 직책이 IT_PROG인 사원의 월급 반환
        -- 메인쿼리로 결과값 전달
        SELECT salary
...
WHERE
    ...
    --
    --          복수행 서브쿼리가 반환한 모든 값 중, *최소값 보다 큰* 사원 필터링
    --salary > ANY BETWEEN 4200 AND 9000
    -- salary > ANY (
    salary > (
        -- 복수 행 비상관 서브쿼리: 직책이 IT_PROG인 사원의 월급 반환
        -- 메인쿼리로 결과값 전달
...
WHERE
    ...
    --          메인쿼리 조건을 수행가능하도록 ANY 키워드 적용
    --
    --          복수행 서브쿼리가 반환한 모든 값 중, *최대값 보다 작은* 사원 필터링
    salary < ANY (
        -- 복수 행 비상관 서브쿼리: 직책이 IT_PROG인 사원의 월급 반환
        -- 메인쿼리로 결과값 전달
        SELECT salary
...
-- 모든 사원조회 
-- (if 5000000보다 큰 월급여를 받는 사원이 한명이라도 있다면...)
SELECT
    last_name,
    department_id,
    salary
